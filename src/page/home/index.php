...
<main class="container px-4">
  ...
  <?php
      ...
      if (isset($_REQUEST["btn"])) {
        ...
        if ($input != "") {
          ...
          $sql = "SELECT * FROM menuitems WHERE item_name LIKE '%" . $input . "%'";
            ...
            while ($row = $result->fetch_assoc()) {
              echo "
                <div class='card w-full'>
                  <a href='src/page/menuitems/" . $row['item_category'] . "/index.php'>
                    <img src='images/" . $row['item_image'] . "' class='card-img-top h-64' alt=''>
                    <div class='card-body flex justify-between items-center'>
                      <h5 class='card-title font-bold'>" . $row['item_name'] . "</h5>
  ...
  <section class="products border-t border-gray-500 my-16">
    ...
    <div class="content-product grid grid-cols-3 gap-x-14 gap-y-10 my-4">
      <?php
      $sql = "SELECT * FROM menuitems GROUP BY item_category ORDER BY item_category ASC";
      $result = $conn->query($sql);
        ...
        echo "
          <div class='card w-full'>
            <a href='src/page/menuitems/" . $row['item_category'] . "/index.php'>
              <img src='images/" . $row['item_image'] . "' class='card-img-top h-64' alt=''>
              <div class='card-body flex justify-between items-center'>
                <h5 class='card-title font-bold'>" . $row['item_category'] . "</h5>
                <a href='#' class='btn btn-outline-info'>Xem thêm</a>
              </div>
            </a>
          </div>
  ...
  <section class="promotions border-t border-gray-500 my-16">
    ...
    <div class="content-party grid grid-cols-1 gap-y-10 my-4">
      <?php
      $sql = "SELECT * FROM promotions LIMIT 3";
      $result = $conn->query($sql);
      $count = 0;
            ...
            <div class='card' style='backround-color: #E6F9FF; border-color: #AED6F1;'>
              <div class='flex justify-between items-center'>
                ...
                <div class='card-body w-5/12' style='border-left: 1px solid #1F618D;'>
                  <h5 class='card-title font-bold'>" . $row['promotion_name'] . "</h5>
                  <p class='card-text'>" . $row['description'] . "</p>
                  <div class='text-body-secondary text-sm mt-2'>" . $row['start_date'] . " đến " . $row['end_date'] . "</div>
